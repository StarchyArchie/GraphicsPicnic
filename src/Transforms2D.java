...
public class Transforms2D extends JPanel {
    /**
     * This main() routine makes it possible to run the class Transforms2D
     * as an application.  It simply creates a window that contains a panel
     * of type Transforms2D.  The program ends when the user closed the
    ...
    public static void main(String[] args) {
        ...
        window.setContentPane(new Transforms2D()); // Show an instance of this class in main window pane
    ...
    /**
     * This constructor sets up a Transforms2D when it is created.  Here, it
    ...
    private void drawScene(Graphics2D g2) {
        ...
        g2.translate(0, 3);
        g2.rotate(Math.PI/6);  // 30 degrees = PI/6 radians    (1 degree = PI/180)
        g2.translate(2, 0);
        drawCoordinateFrame(g2, 10);   // Draw NEW coordinate frame
        drawMainScene(g2);

        // Scene version 2
        g2.setTransform(cs); // Restore "coordinate system" back to last saved transform
        g2.translate(-2, 0);
        g2.scale(0.5, 0.5);
        drawCoordinateFrame(g2, 10);   // Draw NEW coordinate frame
        drawMainScene(g2);
